...
def call(args):
    client = cosmos.CosmosAPI(cosmos_name=args['cosmos_account_name'], resource_group=args['azure_resource_group'])
    ...
    if args['json']:
       ...
       if len(res) > 0:
           print(tabulate([r.values() for r in res], headers=res[0].keys(), tablefmt=args['grid_style']))


def register_subparser(subparser):
    subcommand = subparser.add_parser("cosmos", help="Get cosmos DB entries for a workflow.")
    subcommand.add_argument("workflow-id", help="Cromwell workflow ID.")
    subcommand.add_argument("--failures", action="store_true", help="Return only failed tasks")
    subcommand.add_argument("--grid-style", help="Any valid `tablefmt` for python-tabulate.", default="fancy_grid")
    subcommand.add_argument("--json", help="Print the full JSON objects for Cosmos records", action="store_true")
