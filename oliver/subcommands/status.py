...
def call(args):
    ...
    if "workflow_id" in args and args["workflow_id"]:
        results = list(filter(lambda r: r["Workflow ID"] == args["workflow_id"], results))

    if "workflow_name" in args and args["workflow_name"]:
        results = list(filter(lambda r: r["Workflow Name"] == args["workflow_name"], results))
...
def register_subparser(subparser):
    ...
    subcommand.add_argument("-i", "--workflow-id", type=str, help="Filter by workflow id matching argument.")
    subcommand.add_argument("-n", "--workflow-name", type=str, help="Filter by workflow name matching argument.")
